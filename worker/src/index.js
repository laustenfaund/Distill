// Distill hosted-assistant proxy
//
...
// ever placing the upstream /v1/messages call — not just after, which
// would let a burst of concurrent requests all slip through before any of
// them get counted. /v1/messages/count_tokens is also proxied (passcode +
// origin + rate limit still apply) but not billed against the cap — see
// the note by ASSUMED_FREE_COUNT_TOKENS below.
//
...
// count_tokens has historically been a free Anthropic endpoint (it doesn't
// generate any text). VERIFY this is still true for your models before
// relying on it — if Anthropic ever starts billing it, flip this to false
// so it gets estimated and reserved against the cap like /v1/messages.
const ASSUMED_FREE_COUNT_TOKENS = true;

// Distill always asks for max_tokens: 4096 today, but honor whatever the
// client asks for up to this ceiling rather than hardcoding one value, in
// case that changes — a tampered client request still can't ask for more.
...
export default {
  async fetch(request, env) {
    ...
    if (isCountTokens) {
      return proxyCountTokens(env, cors, model, body);
    }
    return proxyMessages(env, cors, model, body, userId);
  },
};

async function proxyCountTokens(env, cors, model, body) {
  const outgoing = { model, messages: body.messages };
  let upstream;
  try {
  ...
  // ASSUMED_FREE_COUNT_TOKENS: no budget reservation/deduction here. If you
  // set that flag to false above, treat this the same as proxyMessages —
  // estimate, reserve, call, reconcile.
...
async function proxyMessages(env, cors, model, body, userId) {
  ...
  await Promise.all([
    env.USAGE.put(userDayKey, String(userSpent + estimateCents), { expirationTtl: TTL_DAY }),
