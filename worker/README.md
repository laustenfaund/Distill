# Distill assistant proxy

A Cloudflare Worker that stands between the [`hosted/`](../hosted) build of
Distill and Anthropic's API. It holds your real Anthropic API key as a
...
Distill calls two Anthropic endpoints, both proxied here:

- `/v1/messages` — the actual distill pass. Metered against the spending
  caps, same as the other apps' workers.
- `/v1/messages/count_tokens` — the before/after token counts. Still
  passcode + origin + rate-limit gated, but **not** charged against the
  cap, because Anthropic's count_tokens endpoint doesn't generate text and
  has historically been free. `ASSUMED_FREE_COUNT_TOKENS` at the top of
  `src/index.js` flags this assumption — verify it's still true for your
  models before relying on it, and flip it if Anthropic ever changes that.
...
1. **Create the Worker.**
   - Dashboard → **Workers & Pages** → **Create** → **Workers** → give it
     a name (e.g. `distill-assistant` — this becomes part of its URL) →
...
2. **Create two KV namespaces.**
   - Dashboard → **Storage & Databases** → **KV** → **Create a
     ...
     name them anything unique to your account, e.g. `distill-usage` and
     `distill-passcodes`. This namespace name is just a label in your
...
8. **Copy the Worker's URL.** It's shown at the top of the Worker's
   overview page: `https://distill-assistant.<your-subdomain>.workers.dev`.
