// Distill — creator (uncapped) assistant proxy
//
...
// passcode, and proxies both /v1/messages and /v1/messages/count_tokens.
// The difference: this one is for the app owner's own use, so it does NOT
// enforce a spend cap or clamp max_tokens.
//
...
export default {
  async fetch(request, env) {
    ...
    return proxyMessages(env, cors, model, body);
  },
};
...
async function proxyMessages(env, cors, model, body) {
  // No max_tokens clamp, no cost estimate, no spend reservation.
  const outgoing = {
    model,
