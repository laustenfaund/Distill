# Distill creator (uncapped) proxy

A second Cloudflare Worker, separate from [`worker/`](../worker), that
stands between the [`creator/`](../creator) build of Distill and
Anthropic's API. It's the same idea as `worker/` — holds your real
Anthropic API key server-side, gated by a passcode, proxying both
`/v1/messages` and `/v1/messages/count_tokens` — with the caps taken out,
because this build is for you, not for guests.

## How this differs from `worker/`

- **No spend cap.** `worker/` reserves cost against a daily/monthly budget
  before every `/v1/messages` call; this Worker doesn't track cost at all.
  `count_tokens` was never billed against the cap in `worker/` either
  (Anthropic's count_tokens endpoint doesn't generate text), so that part
  is unchanged.
...
- **A generous rate limit stays, as a bug guard, not a budget control.**
  ...
  real interactive session should ever hit it. Raise
  `RATE_LIMIT_PER_MINUTE` / `IP_RATE_LIMIT_PER_MINUTE` in `src/index.js`,
  or remove those checks outright, if you'd rather have zero limits.
...
1. **Create the Worker.**
   - Dashboard → **Workers & Pages** → **Create** → **Workers** → name it
     `distill-creator` (or anything — this becomes part of its URL) →
...
2. **Create two KV namespaces — new ones, not `worker/`'s.**
   - Dashboard → **Storage & Databases** → **KV** → **Create a
     namespace**. Name them something distinct, e.g.
     `distill-creator-usage` and `distill-creator-passcodes`.
...
7. **Copy the Worker's URL** from its overview page:
   `https://distill-creator.<your-subdomain>.workers.dev`. Paste that into
